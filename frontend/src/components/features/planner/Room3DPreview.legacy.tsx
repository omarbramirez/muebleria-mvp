...
// INFRAESTRUCTURA GRÁFICA & CORE
import * as THREE from "three"; // 001 librería base de render
import React, { useEffect, useRef, useMemo } from "react"; // 002 libreria base frontend
import { OrbitControls } from "three/examples/jsm/controls/OrbitControls.js"; // 003 clase de opciones de camara
import { CSS2DRenderer, CSS2DObject } from "three/examples/jsm/renderers/CSS2DRenderer.js"; // 004 clases de conversion HTML/CSS a 3D

// IMPORTACIÓN DE TIPOS E INTERFACES
import {
  usePreferenceWizardStore
} from "@/store/preferenceWizardStore"; // 005 clase de gestión de estado global "Zustand" 
import {Room3DPreviewProps,DraggableItemType} from '@/types/planner/planner' // 006 tipados de propiedades del estado global / tipado de objeto interactivo en render

// IMPORTACIÓN DE LA FÁBRICA DE GABINETES
import { createProceduralCabinet } from "@/utils/CabinetFactory"; // 007 Factory (patron de diseño) de organizadores
import { createProceduralAppliance } from "@/utils/ApplianceFactory"; // 008 Factory (patron de diseño) de electrodomesticos

// IMPORTANCIÓN DE CONSTANTES DE INGENIERÍA
import { WALL_THICKNESS, ENGINEERING_CONSTANTS } from "./config/constants"; // 009 Propieades globales de construccion

// ME QUEDE AQUI, ESTOY ETIQUETANDO CADA COSA PARA ENTENDER QUÉ HACE EXACTAMENTE Y DESPUÉS PODER REFACTORIZAR 

// const Room3DPreview es literalmente la columna vertebral del módulo y lo que se tiene que refactorizar en microservicios 
...
// 4. DEFINICIÓN DE MATERIALES (Síncrono - Seguro para SSR)
    ...
    loader.load(
      ...
      (texture) => {
        ...
        if (materials.cabinetDoor) {
            ...
            materials.cabinetDoor.needsUpdate = true; 
        }
      },
      undefined,
  ...
  const updateMeasurements = (target: THREE.Object3D, wall: THREE.Mesh | null) => {
    ...
    const widthLabel = createLabel(`${Math.round(size.x * ENGINEERING_CONSTANTS.SCALE_FACTOR)} mm`);
    ...
    if (wall) {
      ...
      if (distFloor > 0.1) {
        ...
        // Etiqueta Elevación
        const elevLabel = createLabel(`Elev: ${Math.round(distFloor * ENGINEERING_CONSTANTS.SCALE_FACTOR)}`);
      ...
      if (distLeft > 1) {
        ...
        const lLabel = createLabel(`${Math.round(distLeft * ENGINEERING_CONSTANTS.SCALE_FACTOR)}`);
      ...
      if (distRight > 1) {
        ...
        const rLabel = createLabel(`${Math.round(distRight * ENGINEERING_CONSTANTS.SCALE_FACTOR)}`);
  ...
  useEffect(() => {
    ...
    openings.forEach(op => {
      ...
      const mesh = new THREE.Mesh(
        new THREE.BoxGeometry(op.width / ENGINEERING_CONSTANTS.SCALE_FACTOR, op.height / ENGINEERING_CONSTANTS.SCALE_FACTOR, ENGINEERING_CONSTANTS.OPENING_DEPTH),
        op.type === 'window' ? materials.window : materials.door
      );
      const localX = -wall.userData.length / 2 + (op.distFromStart / ENGINEERING_CONSTANTS.SCALE_FACTOR) + (op.width / ENGINEERING_CONSTANTS.SCALE_FACTOR / 2);
      const localY = (-heightUnits / 2) + (op.sillHeight / ENGINEERING_CONSTANTS.SCALE_FACTOR) + (op.height / ENGINEERING_CONSTANTS.SCALE_FACTOR / 2);
    ...
    layoutItems.forEach(item => {
      ...
      const zOffset = (WALL_THICKNESS / 2) + (item.depth / ENGINEERING_CONSTANTS.SCALE_FACTOR / 2);
  ...
  useEffect(() => {
    ...
    const processPointerMove = (evt: PointerEvent) => {
      ...
      if (type === 'gas' || type === 'installation' || type === 'furniture' || type === 'appliance') {
        ...
        if (wallHits.length > 0) {
          ...
          const limitX = (wallLen / 2) - (w / 2) - ENGINEERING_CONSTANTS.CORNER_SAFETY_MARGIN;
        ...
        if (hits.length > 0) {
          ...
          // Clamping estricto
          const limitX = (wallLen / 2) - (w / 2) - ENGINEERING_CONSTANTS.CORNER_SAFETY_MARGIN; // También aplicamos margen aquí
    ...
    const handlePointerUp = (e: PointerEvent) => {
      ...
      if (type === 'furniture' && onLayoutUpdate) {
        ...
        if (targetWall) {
          onGasUpdate({ ...gasConfig, x: Math.round((mesh.position.x + wallLen / 2) * ENGINEERING_CONSTANTS.SCALE_FACTOR), z: Math.round((mesh.position.y + wallH / 2) * ENGINEERING_CONSTANTS.SCALE_FACTOR), wallIndex });
        } else {
          onGasUpdate({ ...gasConfig, x: Math.round(mesh.position.x * ENGINEERING_CONSTANTS.SCALE_FACTOR), z: Math.round(mesh.position.z * ENGINEERING_CONSTANTS.SCALE_FACTOR), wallIndex: -1 });
        }
      }
      else if (type === 'installation' && onInstallationUpdate) {
        const inst = installations.find(i => i.id === id);
        if (inst && targetWall) {
          onInstallationUpdate({ ...inst, distFromStart: Math.round((mesh.position.x + wallLen / 2) * ENGINEERING_CONSTANTS.SCALE_FACTOR), heightFromFloor: Math.round((mesh.position.y + wallH / 2) * ENGINEERING_CONSTANTS.SCALE_FACTOR), wallIndex });
        } else if (inst) {
          onInstallationUpdate({ ...inst, distFromStart: Math.round(mesh.position.x * ENGINEERING_CONSTANTS.SCALE_FACTOR), heightFromFloor: Math.round(mesh.position.z * ENGINEERING_CONSTANTS.SCALE_FACTOR), wallIndex: -1 });
        }
      }
        ...
        if (op) {
          onOpeningUpdate({ ...op, distFromStart: Math.round((mesh.position.x + wallLen / 2 - w / 2) * ENGINEERING_CONSTANTS.SCALE_FACTOR), sillHeight: Math.round((mesh.position.y + wallH / 2 - h / 2) * ENGINEERING_CONSTANTS.SCALE_FACTOR), wallIndex });
        }
      }
