...
import {
  ...
  Trash2,
  AlertCircle,
  X, // Reemplazamos XIcon por X de lucide
} from "lucide-react";
...
interface InstallationPoint {
  ...
  subtype: string;
  x: number; // Distancia X (Posición horizontal)
  y: number; // Profundidad (para tomas de piso/drenajes)
  z: number; // Altura Z
  notes?: string;
  // Propiedades adicionales para plomería:
  hasHotWater?: boolean;
...
interface GasConfig {
    required: boolean;
    type: 'natural' | 'lp'; // Asumimos que hay un subtipo 'lp' (licuado) o similar para escalabilidad
    x: number;
    z: number;
}

// --- VALORES DE INICIALIZACIÓN (Defaults si el store está vacío) ---
...
const Compatibility = () => {
  ...
  // 1. HYDRATION (Lectura)
  const [points, setPoints] = useState<InstallationPoint[]>(values.installation_points || INITIAL_INSTALLATIONS);
  const [gasConfig, setGasConfig] = useState<GasConfig>(values.gas_config || INITIAL_GAS_CONFIG);
  ...
  const addPoint = (type: InstallationType) => {
    const defaultPoint: InstallationPoint = {
      ...
      x: 100, // cm
      y: 0,
      z: type === 'electrical' ? 110 : 55, // 110cm para eléctrica, 55cm para plomería
  ...
  // 3. SYNCHRONIZATION (Escritura en el Store Global)
  useEffect(() => { 
    // Guardamos los puntos de instalación
    setValue('installation_points', points);
  }, [points, setValue]);
  
  useEffect(() => { 
    // Guardamos la configuración de gas
    setValue('gas_config', gasConfig);
  }, [gasConfig, setValue]);
    ...
    <div className="p-6 bg-white flex flex-col gap-6">
      ...
      <section className="border rounded-xl overflow-hidden">
        ...
        <div className="p-4 space-y-4">
          ...
          {points.filter(p => p.type === 'electrical').map((point, index) => (
